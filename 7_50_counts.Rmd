...
# Example with 50 counts

## Generate 50 counts and compute the total count

```{r}
set.seed(328328)
N <- sort(c(0, 0, 0, 1, 2, 3, 8, 15, 24, 60, round(runif(40, 0, 1000))))
t <- sum(N)
N
t
```
...
```{r, message = F}
# Algorithm 1
result1 <- data.frame(n1 =multi_1[1,], n2 = multi_1[12,], n3 = multi_1[25,], n4 = multi_1[38,], n5 = multi_1[50,])
results1 <- stack(result1)

truevalue <- data.frame(ind = c("n1", "n2", "n3", "n4", "n5"), true = c(N[1], N[12], N[25], N[38], N[50]))

e1a1 <- ggplot(data = results1, aes(values, fill = ind)) + 
  geom_histogram() + 
  facet_wrap(~ind, scales = "free") + 
  geom_vline(data = truevalue, aes(xintercept = true)) + 
  ggtitle("Example 1 - Algorithm 1")

e1a1

ggsave("e1a1.png", e1a1, width = 7, height = 4, units = "in")
```

```{r, message = F}
# Algorithm 2
result2 <- data.frame(n1 =multi_2[1,], n2 = multi_2[12,], n3 = multi_2[25,], n4 = multi_2[38,], n5 = multi_2[50,])
results2 <- stack(result2)

e1a2 <- ggplot(data = results2, aes(values, fill = ind)) + 
  geom_histogram() + 
  facet_wrap(~ind, scales = "free") + 
  geom_vline(data = truevalue, aes(xintercept = true)) + 
  ggtitle("Example 2 - Algorithm 2")

e1a2 
ggsave("e1a2.png", e1a2,  width = 7, height = 4, units = "in")
```

```{r, message = F}
# Algorithm 3
result3 <- data.frame(n1 =multi_3[1,], n2 = multi_3[12,], n3 = multi_3[25,], n4 = multi_3[38,], n5 = multi_3[50,])
results3 <- stack(result3)

e1a3 <- ggplot(data = results3, aes(values, fill = ind)) + 
  geom_histogram() + 
  facet_wrap(~ind, scales = "free") + 
  geom_vline(data = truevalue, aes(xintercept = true)) + 
  ggtitle("Example 1 - Algorithm 3")

e1a3 
ggsave("e1a3.png", e1a3, width = 7, height = 4, units = "in")
```
